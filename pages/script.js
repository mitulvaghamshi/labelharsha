...
window.addEventListener("DOMContentLoaded", () => {
    ...
    Inventory.Catalog.populate("tall");
    productsContainer.innerHTML += `<div style="height: 35px;"></div>`;
    Inventory.Catalog.populate("wide");
});
...
Inventory.Catalog = ((root) => {
    ...
    return {
        populate: (tag) => {
            if (tag === "tall") populateCatalog(tag, dataSet1);
            if (tag === "wide") populateCatalog(tag, dataSet2);
        },
    };
})(productsContainer);
